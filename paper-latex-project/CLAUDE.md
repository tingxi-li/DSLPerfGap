...
.
├── CLAUDE.md                                # This file
├── main.tex                                 # Top-level entry; acmart[sigconf,review,anonymous]
├── references.bib                           # BibTeX bibliography (loaded at main.tex:163)
├── acmart.cls / ACM-Reference-Format.bst    # ACM style files (vendored)
├── main.pdf                                 # Built output (commit-tracked)
├── PROGRESS.md                              # Author-side section status tracker (working note)
├── profile.csv                              # COPY of ../ViperBench/results/profile.csv
├── slow_kernels.csv                         # COPY of ../ViperBench/results/slow_kernels.csv
├── GPU Kernel Performance Analysis Report.md  # COPY of ../AKO4ALL/context/<same>
...
> **Note on duplicated data files.** `profile.csv`, `slow_kernels.csv`, and `GPU Kernel Performance Analysis Report.md` are copies of files in the parent repo. They are NOT regenerated by the build — if the source-of-truth files in `../ViperBench/results/` or `../AKO4ALL/context/` change, these copies go stale. Re-sync manually.
...
Current build uses `\documentclass[sigconf,review,anonymous]{acmart}` (anonymized for ASE 2026 review). The commented venue lines at `main.tex:11-16` will be re-enabled for camera-ready.
...
- **Mitigation (`tex/mitigation.tex`):** Five fixed kernels (`tab:mitigation`) feed from `../AKO4ALL/results/optimization_results.csv` rows. Conv arm also uses `../experiments/results/<gpu>/conv_mitigation*.csv`. PyTorch baseline footnote at `tab:mitig:conv` (≈9% drift from `tab:conv`) is intentional and documents clock variation across runs.
