...
\Cref{tab:summary} reports the GEMM category range; the extremes are complementary by shape, not merely by DSL average---Triton is weakest on the $16384^2$ square matmul (32.8\%) and strongest on the $128\times2048^2$ batched shape (92.9\%), while TileLang inverts this exactly (80.0\% on the square matmul, 7.0\% on the same batched shape), before both are eclipsed by TileLang's $548.6\%$ fusion win on \texttt{fused\_linear\_activation}.
Section \Cref{sec:analysis:autotune} attributes Triton's $16384^2$ collapse to two compounding, A100-specific gaps: the default \texttt{@triton.autotune} grid omits the L2-swizzle and large-shape configurations this shape needs (RC2a), and its generic \texttt{tl.dot} lowering lacks cuBLASLt's cache-blocking heuristics for the resulting ${\approx}1.6$~GB working set, leaving it DRAM-bound (85.9\% DRAM throughput, 49.4\% L2 hit rate) against cuBLAS's cache-resident profile (28.8\%/80.5\%) (RC2b).
The fusion win is architectural rather than a compiler artifact---TileLang collapses the matmul-plus-activation pass into a single kernel launch, avoiding PyTorch's intermediate materialization---so it should not be read as evidence the gap closes on shapes without a fusible second op.
On the GH200 the same GEMM shapes reorder in both DSLs' favor (Triton 56--170\%, TileLang 60--163\%, \Cref{tab:summary}), consistent with RC2a/RC2b being A100 tuning- and cache-blocking gaps rather than limitations intrinsic to either DSL.
