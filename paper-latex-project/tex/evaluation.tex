...
On the GH200, Triton convolution stays comparably weak (12.1\% at $3\times3$, $\le$9.8\% at the larger filters), but TileLang convolution is far stronger than on the A100 (per-filter 32.9--71.7\% across $1\times1$--$7\times7$ in \texttt{conv\_filters.csv}, median 54.1\% in \cref{tab:summary}), so the TileLang convolution collapse is largely an Ampere-specific scheduling artifact rather than intrinsic to the DSL---the persistent residual is the \emph{Triton} convolution gap, which holds on both architectures.
