...
\begin{figure}[t]
  ...
  \caption{Library efficiency (\%) of Triton and TileLang kernels relative to cuBLAS/cuDNN
    ...
    Attention excluded.\protect\footnotemark}
  \label{fig:overview}
\end{figure}
\footnotetext{\label{fn:attention}Triton's tiled FlashAttention-2~\cite{dao2023flashattention2} and TileLang's untiled $O(n^2)$ kernel implement algorithmically different computations, so no single library-efficiency number is a valid DSL-versus-library comparison.}
...
\begin{table}[t]
  ...
  \caption{Median library efficiency (\%) per kernel category and DSL on the A100-SXM4 and GH200
    ...
    Attention excluded from Overall.\protect\footnotemark[\getrefnumber{fn:attention}]}
  \label{tab:summary}
  ...
  \end{tabular}}%
  {\footnotesize
    $^\ddagger$ LayerNorm 0.1\% is a serialization artifact (RC0; \Cref{sec:analysis:jit}), not a data error; RMSNorm 851.8\% is an unfair-baseline artifact (PyTorch eager does not fuse normalization passes) --- see body text.\par
    $^\S$ Overall figures are medians excluding attention; the Triton geomean ($\sim$126\%) is inflated by fusion and unfused-eager-baseline outliers, so we report the median as the more representative summary.}
\end{table}
\footnotetext[\getrefnumber{fn:attention}]{Triton's tiled FlashAttention-2~\cite{dao2023flashattention2} and TileLang's untiled $O(n^2)$ kernel implement algorithmically different computations, so no single library-efficiency number is a valid DSL-versus-library comparison.}
