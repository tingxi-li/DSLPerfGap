...
\paragraph{Attention}
Attention is excluded from \Cref{fig:overview} and \Cref{tab:summary} because Triton's tiled FlashAttention-2~\cite{dao2023flashattention2} and TileLang's untiled $O(n^2)$ kernel implement algorithmically different computations, so no single library-efficiency number is a valid DSL-versus-library comparison.
