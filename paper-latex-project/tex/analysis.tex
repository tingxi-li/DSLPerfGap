...
This algorithmic structure, however, is not where the cost lies: on the A100, the performance gain from \texttt{T.reduce} comes from better memory-latency hiding---\texttt{warp\_stall\_long\_scoreboard} drops from 59.56 cycles to ${\approx}0$---not from removing barriers, since \texttt{warp\_stall\_barrier} is already ${\approx}0$ in both the \texttt{T.serial} and \texttt{T.reduce} kernels.
Replacing the \texttt{T.serial} loop with a single \texttt{T.reduce} call thus removes the dominant latency source: in our experiments, LayerNorm drops from 1090~ms to 0.89~ms ($1224\times$ improvement, \cref{sec:mitig:norm}).
