...
\begin{figure}[t]
\begin{lstlisting}
# (a) T.serial: sequential accumulate -- idiomatic but slow
mean_val[0] = T.float32(0)
for j in T.serial(n):       # no inter-thread parallelism
    mean_val[0] = mean_val[0] + row[j]
mean_val[0] = mean_val[0] / T.float32(n)

# (b) T.reduce: parallel butterfly -- one-line fix
T.reduce(row, mean_val, "sum", dim=0, clear=True)
mean_val[0] = mean_val[0] / T.float32(n)
\end{lstlisting}
\caption{RC0a in TileLang LayerNorm: swapping four lines of \texttt{T.serial}
  accumulation (a) for a single \texttt{T.reduce} call (b) eliminates
  \texttt{warp\_stall\_long\_scoreboard} stalls on the A100 and yields
  a $1347\times$ latency improvement (\cref{sec:mitig:norm}).}
