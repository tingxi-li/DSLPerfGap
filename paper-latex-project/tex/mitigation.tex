...
\begin{table}[t]
  ...
  $^\S$ On the A100 stack (TileLang 0.1.6.post1) the in-tree TileLang Softmax/LogSoftmax variant's 64\,KB full-row shared-memory cache collapses occupancy ($\rho{=}0.006$/$0.028$); we report the streaming variant (\cref{sec:mitig:summary}), which reaches parity on both architectures; its cliff is omitted as the in-tree comparison does not transfer.
