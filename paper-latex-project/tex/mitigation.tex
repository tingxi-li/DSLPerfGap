...
\begin{table}[t]
  ...
  $^\S$ On the A100 stack (TileLang 0.1.6.post1) the in-tree TileLang Softmax/LogSoftmax variant's 64\,KB full-row shared-memory cache collapses occupancy ($\rho{=}0.006$/$0.028$); we report the \texttt{opt\_kernels} streaming variant, which reaches parity on both architectures; its cliff is omitted as the in-tree comparison does not transfer.
  \texttt{logsumexp} is omitted from the recovered family: its optimized kernel register-spills on \texttt{sm\_90} (GH200; $\rho<0.02$, $E_\text{lib}=1.0\%$) but \emph{not} on \texttt{sm\_80} (A100: $0$ spill, $93$ regs, $E_\text{lib}=582\%$)---an architecture-specific RC3-class residual discussed in \cref{sec:mitig:summary} and \cref{tab:mitigation}.}
\end{table}
...
Per-filter optimized efficiencies range from 75.5\% at $1\times1$ to 36.1\% at $7\times7$ (all correct); closing this tuning gap and adding in-DSL Winograd support are future work.
...
\begin{table}[t]
  ...
  traffic, $12\%$ occupancy) $\to$ $1.0\%$; even retuned to the GH200-optimal
  ...
  occupancy), confirming the $581.7\%$ is genuine and the spill is sm\_90-specific.
