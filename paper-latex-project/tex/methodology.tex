...
Kernels are drawn from TritonBench~\cite{li2025tritonbench}, the TileLang example repository~\cite{wang2025tilelang}, and our own implementations for configurations absent from both sources; we retain only forward-pass kernels with stable library baselines and balanced five-category coverage, excluding sparse and dynamic-shape operators.
...
For each workload, we compare DSL implementations against the corresponding vendor-library path exposed through PyTorch.
...
We report these results separately because the Triton and TileLang implementations are not algorithmically identical to the PyTorch baseline.
...
For each workload and input configuration, we measure host-synchronized GPU execution time using \texttt{time.perf\_counter()} bracketed by \texttt{torch.cuda.synchronize()} calls.
...
Clocks are pinned to a fixed frequency throughout; run-to-run relative standard deviation is $\leq 0.9\%$ (settings in \cref{app:repro:meas}).

\paragraph{Hardware performance counters}
To support root-cause analysis, we collect Nsight Compute~\cite{nsightcompute2024} profiles grounded in seven counters (definitions in \cref{app:repro:meas}), each from a single execution with stability verified within 2\% across five runs.

\paragraph{Correctness}
Before any timing, we validate every DSL kernel against its library baseline at per-dtype tolerances (\texttt{fp32}~$10^{-5}$, \texttt{fp16}~$10^{-3}$, \texttt{bf16}~$10^{-2}$, relaxed $2\times$ for reductions) and an edge-case suite (NaN/Inf/denormal/all-equal) across all 22 kernels with 0 crashes; the RQ3-mitigated kernels revalidate 5/5.
...
Every optimized kernel is validated provenance-agnostically against the same per-dtype tolerances (5/5 pass) and Nsight Compute counters, and recovered performance is reported as a lower bound on user-space-recoverable gain.
...
To anchor the RQ3 heuristic in a baseline-independent signal, we model each kernel's essential work---bytes moved for memory-bound kernels or FLOPs for compute-bound kernels---and divide the achieved work-rate by the relevant hardware peak (HBM bandwidth or FP16 Tensor-Core throughput) to obtain its roofline fraction.
...
\paragraph{Hardware}
We measure on two primary architectures.
The first is an NVIDIA A100-SXM4-40GB (Ampere, \texttt{sm\_80}; 108 SMs, 40\,GB HBM2e, $\sim$1.5\,TB/s, 400\,W), on which we measure suite magnitude (\cref{sec:evaluation}) and root-cause counters (\cref{sec:analysis}).
The second is an NVIDIA GH200 (Grace Hopper, \texttt{sm\_90}; 96\,GB HBM3, $\sim$4.0\,TB/s, $\sim$989.5\,TFLOP/s FP16 Tensor-Core), on which we run the evaluation-gap demonstration (\cref{sec:eval:passesslow}) and validate the roofline heuristic (\cref{tab:roofline}).
Full hardware specifications and clock-lock settings are in \cref{app:repro:stack}.

\paragraph{Software}
We use PyTorch~2.8.0+cu128, Triton~3.4.0, TileLang~0.1.6.post1, bundled cuDNN, and Nsight Compute~2026.2.0, with all versions pinned in a reproducibility manifest distributed with the artifact; complete version list is in \cref{app:repro:stack}.
