...
Five near-parity element-wise kernels (\texttt{leaky\_relu}, \texttt{swiglu}, \texttt{logsumexp}, \texttt{cross\_entropy}, \texttt{matrix\_transpose}) exceeded 100\% on both DSLs at the large shape and are omitted from this table; their main-paper medians appear in \Cref{tab:summary}.
...
\begin{table}[h]
  ...
  \begin{tabular}{llrr}
    ...
    \texttt{matmul}              & $16384\!\times\!16384$ FP16          & 31.7\%  & 56.4\% \\
    \texttt{batched\_matmul}     & $128\!\times\!2048\!\times\!2048$ FP16 & 58.2\%  & 10.7\% \\
    \texttt{linear\_activation}  & $(1,2048,4096)$ FP16                 & 51.9\%  & \textbf{199.7\%}$^{\dagger}$ \\
    \midrule
    \multicolumn{4}{l}{\textit{Attention (excluded from main comparison; see \Cref{sec:eval:gemm})}} \\
    \texttt{attention}           & QKV $(8,32,2048,128)$ FP32           & \textbf{1918\%}$^{\ddagger}$ & 69.2\% \\
    \midrule
    \multicolumn{4}{l}{\textit{Convolution}} \\
    \texttt{conv2d}              & $32\!\times\!256\!\times\!128^2$, $3\!\times\!3$ FP16 & 34.9\% & 9.5\% \\
    \midrule
    \multicolumn{4}{l}{\textit{Normalization}} \\
    \texttt{layer\_norm}         & $8192\!\times\!8192$ BF16            & 94.5\%  & 0.3\%  \\
    \texttt{rms\_norm}           & $8192\!\times\!8192$ FP16            & \textbf{1099\%}$^{\S}$ & 5.3\% \\
    \midrule
    \multicolumn{4}{l}{\textit{Element-wise / Reduction}} \\
    \texttt{add}                 & 64M FP16                             & 103.5\% & 77.3\% \\
    \texttt{mul}                 & 64M FP16                             & 69.7\%  & 69.1\% \\
    \texttt{relu}                & $16384\!\times\!16384$ FP16          & 103.4\% & 68.7\% \\
    \texttt{softmax}             & $(4096,32768)$ FP16                  & 97.7\%  & 20.1\% \\
    \texttt{log\_softmax}        & $(4096,32768)$ FP16                  & 76.2\%  & 16.9\% \\
    \texttt{argmax}              & $(8192,32768)$ FP16                  & 71.4\%  & 6.4\%  \\
    \texttt{max\_reduction}      & $(8192,32768)$ FP16                  & 12.8\%  & 5.7\%  \\
    \texttt{mean\_reduction}     & $(8192,32768)$ FP32                  & 99.3\%  & 16.0\% \\
    \texttt{embedding}           & $(131072,1024)$ idx FP16             & \textbf{405.8\%}$^{\P}$ & 57.9\% \\
    \texttt{index\_select}       & $(65536,2048)$ idx FP16              & 85.2\%  & 97.2\% \\
    \bottomrule
  \end{tabular}}
  \smallskip
