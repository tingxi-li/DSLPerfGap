...
RC1 (\cref{sec:analysis:vec}) shows that convolution performance is limited in part by the failure to emit wide \texttt{LDG.128} loads for strided spatial accesses. A direct compiler remedy is to extend alias and layout analysis to recognize the convolution access pattern and emit vectorized loads when the contiguous dimension is aligned (e.g., multiples of 8 for FP16). This change targets code generation rather than the programming model, and therefore does not require changes to the user-facing API.
