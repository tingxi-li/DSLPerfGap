...
\textbf{The root-cause taxonomy guides debugging.}
For TileLang reductions and normalization, replace \texttt{T.serial} with \texttt{T.reduce} (RC0a, the dominant cost---exposed memory latency, not synchronization) and inspect register spill (RC3, which on our hardware appears in normalization, not convolution); for convolution, check \texttt{LDG.128} vectorization (RC1) and whether the tuning space is merely under-populated (RC2a, fixable by expanding the search) or needs a broader strategy (RC2b), with Winograd (RC4) only a small ($\approx$2--3\%) contributor---a compact checklist mirroring \cref{sec:analysis,sec:mitigation}.
