...
First, the evaluation gap is real and large: naive-but-correct kernels pass the gate while running 17--1293$\times$ slower than the library, and the dominant existing benchmarks neither gate on performance nor cover enough of the space to catch it.
Second, the hidden gap is highly structured.
Much of it is an \emph{authoring} artifact---a single reduction-primitive change (\texttt{T.serial}$\rightarrow$\texttt{T.reduce}) with native-dtype I/O closes a 1500$\times$ cliff on TileLang normalization and brings it to or above library parity---while a residual gap on convolution is structural, reflecting cuDNN's implicit-GEMM maturity (absent Winograd support contributes only $\approx$2--3\%).
